  ...
  <div class="app-container percentVisonManage">
    ...
    <el-form>
      <ul class="heard-search">
        <li>
          <span class="rem_tit">在线人数版本名称：</span>
          <span class="rem_con">
            <el-input v-model="onlinePeople.name" :disabled="disabledALL" placeholder="请输入在线版本名称" maxlength="15"></el-input>
          </span>
        </li>
        <li>
          <span class="rem_tit">在线人数版本状态：</span>
          <span class="rem_con">
              <el-select v-model="onlinePeople.disable" :disabled="disabledALL" value-key="name" placeholder="请选择">
      ...
      >
        <el-table-column width="35">
          <template slot-scope="scope">
            <el-radio :label="scope.row.id" v-model="online_rowDataId" class="hidTextRadio"></el-radio>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="name" sortable align="center" label="在线人数版本名称" show-overflow-tooltip></el-table-column>
        <el-table-column prop="description" align="center" label="版本说明" show-overflow-tooltip></el-table-column>
        <el-table-column prop="disable" align="center" sortable label="在线人数版本状态"  show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-table-column prop="createdDate" align="center" sortable label="创建时间" show-overflow-tooltip></el-table-column>
        <el-table-column prop="lastModifiedBy" align="center" label="修改人" show-overflow-tooltip></el-table-column>
        <el-table-column prop="lastModifiedDate" align="center" sortable label="修改时间" show-overflow-tooltip></el-table-column>
      </el-table>
    </div>
    <!-- 分页 -->
    ...
    <el-dialog :visible.sync="enableOrUseDialog" :title="title_Name" width="350px" :top="dialogMt">
      <p style="text-align:center;">
        是否确定{{title_Name}}版本名称为
        <span style="color:red">"{{dataName}}"</span>的在线人数？
      </p>
    ...
    <el-dialog :visible.sync="addPercentDialog" title="新增" width="410px" :top="dialogMt" :close-on-click-modal="false">
      <div class="dialog-box">
        ...
        <p style="color:red;text-align: center;">注：版本新增成功后，该版本默认取A版本配置的在线人数</p>
      </div>
    ...
    <el-dialog :visible.sync="editPercentTableDialog" :title="editNames" class="percentVisonManage-deitpercent" width="900px" :top="dialogMt" :close-on-click-modal="false">
      <div class="dialog-box">
        ...
        >
          ...
          <el-table-column prop="onlineVer" align="center" label="选择二级彩种在线人数版本号"   show-overflow-tooltip>
            <template slot-scope="scope">
              <el-select  style="background:#00ffff" placeholder="请选择" v-if="editNames == '编辑'"  value-key="value" v-model="scope.row.onlineVer" @change="verChange(scope.row,scope.row.onlineVer,scope.$index)">
                <el-option v-for="(item,index) in scope.row.detailList" :key="index" :label="item.onlineVer" :value="item.onlineVer"></el-option>
              </el-select>
              <span v-if="editNames == '查看详情'">{{scope.row.onlineVer}}</span>
          </template>
          </el-table-column>
          <el-table-column prop="description" align="center" label="在线人数版本说明" show-overflow-tooltip>
            <template slot-scope="scope">
    ...
    <el-dialog :visible.sync="addPercentMarkDialog" title="编辑说明" width="400px" :top="dialogMt" :close-on-click-modal="false">
      <div class="dialog-box">
        <p>
          <span class="dialog-label-item online-people">
            <span class="required "></span>在线人数版本名称：
          </span>
...
import { getTableList,onlinePeopleDisable,addPeople,editPeople,saveEdit,mergeForDescription } from '@/api/lotteryCore/lc_info/lc_peopleOnlineManage'
...
export default {
  ...
  computed:{
    ...mapState({
      allData: state => state.lc_peopleOnline.allData,
      allDataCount: state => state.lc_peopleOnline.allData_total,
      currentPageSize: state => state.lc_peopleOnline.currentPageSize,
    }),
    size: {
      get(){
        return this.$store.state.lc_peopleOnline.size;
      },
      set(item){
        return  item;
      }
    ...
    onlinePeople:{
      get(){
        return this.$store.state.lc_peopleOnline.onlinePeople;
      },
      set(item){
        return  item;
      }
    ...
    disableArr:{
      get(){
        return this.$store.state.lc_peopleOnline.disableArr;
      },
      set(item){
        return  item;
      }
    ...
    page: {
      get(){
        return this.$store.state.lc_peopleOnline.page;
      },
      set(item){
        return  item;
      }
  ...
  methods:{
    verChange(row,id,index) { // 监听弹框表格 赔率下拉事件
      ...
      let tempVer = {};
       detailList.forEach(element => {
        if(element.onlineVer ==id ){
          tempVer = element;
        }
      });
      this.editTableData[index].description = tempVer.description;
      this.editTableData[index].lotteryOnlineVerId = tempVer.lotteryOnlineVerId;
    },
    ...
    searchTableData(val){  // 查询列表
      this.tableRowData = "";
      this.online_rowDataId = "";
      if(!val) {
        val = 1;
      } 
      this.loading = true; // 请求接口时loading启用
      this.disabledALL=true; // 控制全局不可编辑
      this.$store.dispatch("getPeopleOnlinePage",parseInt(val));
      let data = {
        name: this.$store.state.lc_peopleOnline.onlinePeople.name,
        disable: this.$store.state.lc_peopleOnline.onlinePeople.disable,
        page:this.page-1,
        size:this.size
      }
      if(data.disable){
      ...
      //  查询接口
      getTableList(data).then(resdata => {
        let res = resdata.data;
        if(res.success == 1) {
          this.$store.dispatch("getTableListData",res.content.content)
          ...
          this.$store.dispatch('getPeopleOnlineTableHeight', currentPageSize)  
    ...
    resetTable() {  // 重置
      this.$store.state.lc_peopleOnline.onlinePeople={
        name:"", 
        disable:""
      }
    },
    enableOrUse(val,h) { // 启用弹框
      let indexId = this.online_rowDataId;
      if (!indexId) {
    ...
    confirmOkOrStop(val) {  // 确认按钮--启用    
      let data = {
        id:this.online_rowDataId,
        disable:this.disableType
      }
       // 禁用与启用接口
        onlinePeopleDisable(data).then(resdata => {
        let res = resdata.data;
          if(res.success == 1) {
    ...
    editPercent(val,h) {  // 编辑     
      ...
      let data = {
        id:this.online_rowDataId,
        withAboutDetail:true
      }
      editPeople(data).then(res => {
        let par = res.data;
        if(par.success == 1){
    ...
    saveEditTable() {  // 保存编辑
      ...
      let info = this.editTableData;
      debugger
      info.forEach(itme=>{

        sumOddsLargeVerId.push(itme.lotteryOnlineVerId)
        
      })
      let data = {
        lotteryOnlineVerIdList:sumOddsLargeVerId, // 二级彩种赔率版本号id列表
        id:this.online_rowDataId,// 大版本id【OddsLargeVer.id】
      }
    ...
    handleSizeChange(val){
      this.$store.dispatch('getPeopleOnlineSize', val);
      this.searchTableData();
    }
  },
  created() {
    findByCode({code: "DISABLE"}).then(res => {  // 初始化下拉列表
    this.$store.state.lc_peopleOnline.disableArr =[{value:"",name:"全部"}] 
      let resData = res.data;
      if(resData.success == 1) {
         let by_theStatus = resData.content['DISABLE']; // 初始化启用禁用
                by_theStatus.forEach(element => {
                  this.$store.state.lc_peopleOnline.disableArr.push(element);
                });  
      }
    })
    if(this.$store.getters.alreadyClose.includes(this.$store.getters.currentView)){
      if(this.$store.getters.currentView == this.$route.name) {
        ...
        this.$store.state.lc_peopleOnline.allData = []
        this.$store.state.lc_peopleOnline.allData_total = 0;
        this.$store.state.lc_peopleOnline.page = 1;
        this.$store.state.lc_peopleOnline.size = 1000;
        this.$store.state.lc_peopleOnline.disableArr =[{value:"",name:"全部"}] 
        this.$store.state.lc_peopleOnline.onlinePeople ={name:"", disable:""}, // 头部搜索栏
        this.$store.dispatch('getPeopleOnlineTableHeight', 2)
      }
    }else {
      if(this.$store.state.lc_peopleOnline.allData.length > 0) {
       // debugger
        this.getList()
      }
    }
